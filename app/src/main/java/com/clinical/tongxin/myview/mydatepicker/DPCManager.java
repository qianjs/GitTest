package com.clinical.tongxin.myview.mydatepicker;
...
public final class DPCManager {
    ...
    private static final HashMap<String, Set<SignInEntity>> DECOR_CACHE_BG = new HashMap<>();
    private static final HashMap<String, Set<String>> DECOR_CACHE_WSI = new HashMap<>();
    private static final HashMap<String, Set<String>> DECOR_CACHE_WC = new HashMap<>();
    private static final HashMap<String, Set<String>> DECOR_CACHE_WP = new HashMap<>();
    private static final HashMap<String, Set<String>> DECOR_CACHE_C = new HashMap<>();
     ...
     *
     * @param signInEntities 日期列表 List of date
     */
    public void setSignInBG(List<SignInEntity> signInEntities) {
        setBG(signInEntities, DECOR_CACHE_BG);
    }
    ...
    private void setBG(List<SignInEntity> signInEntities, HashMap<String, Set<SignInEntity>> cache) {
        for (SignInEntity signInEntity : signInEntities) {
            int index = signInEntity.getDate().lastIndexOf("-");
            String key = signInEntity.getDate().substring(0, index).replace("-", ":");
            Set<SignInEntity> days = cache.get(key);
            if (null == days) {
                days = new HashSet<>();
            }
            signInEntity.setDate(signInEntity.getDate().substring(index + 1, signInEntity.getDate().length()));
            days.add(signInEntity);
            cache.put(key, days);

            String tempStr = signInEntity.getDate();
    ...
    private DPInfo[][] buildDPInfo(int year, int month) {
        ...
        Set<SignInEntity> decorBG = DECOR_CACHE_BG.get(year + ":" + month);
        ...
        for (int i = 0; i < info.length; i++) {
            for (int j = 0; j < info[i].length; j++) {
                ...
                if (null != decorBG ){
                    for (SignInEntity signInEntity :decorBG){
                        if (tmp.strG.equals(signInEntity.getDate())){
                            tmp.isDecorBG = true;
                            if ("0".equals(signInEntity.getType()))tmp.isWaitSignIn =true;
                            if ("1".equals(signInEntity.getType()))tmp.isWaitConfirm =true;
                            if ("2".equals(signInEntity.getType()))tmp.isWaitPay =true;
                            if ("3".equals(signInEntity.getType()))tmp.isComplete =true;
                            break;
                        }
                    }
