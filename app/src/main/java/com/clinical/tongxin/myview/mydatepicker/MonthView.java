...
//                }
    ...
    private void draw(Canvas canvas, Rect rect, DPInfo info) {
        ...
        if (isFestivalDisplay) drawFestival(canvas, rect, info, info.isFestival);
...
//        }
        ...
        if(isSelChangeColor&&mCManager.getSelDateList().contains(info.strG)){ //选中了变色
            mPaint.setColor(selChangeTextColor);
        }
...
//        }
        ...
        if (info.isWaitSignIn || info.isWaitConfirm || info.isWaitPay || info.isComplete){
            //mPaint.setColor(Color.WHITE);
        }
        canvas.drawText(info.strG, rect.centerX(), y, mPaint);
    }

    private void drawFestival(Canvas canvas, Rect rect, DPInfo info, boolean isFestival) {
        mPaint.setTextSize(sizeTextFestival);
        if ( info.isWaitSignIn){
            mPaint.setColor(Color.RED);
            canvas.drawText("未签到", rect.centerX(),
        ...
        if (info.isWaitConfirm){
            ...
            canvas.drawText("未确认", rect.centerX(),
                    rect.centerY() + offsetYFestival1, mPaint);
            return;
        }
        if (info.isWaitPay){
            mPaint.setColor(Color.rgb(255,165,0));
            canvas.drawText("未支付", rect.centerX(),
                    rect.centerY() + offsetYFestival1, mPaint);
            return;
        }
        if (info.isComplete){
            mPaint.setColor(Color.GREEN);
            canvas.drawText("已支付", rect.centerX(),
                    rect.centerY() + offsetYFestival1, mPaint);
            return;
        }
        if (isFestival) {
        ...
        if (info.strF.contains("&")) {
            String[] s = info.strF.split("&");
            String str1 = s[0];
            ...
            if (mPaint.measureText(info.strF) > rect.width()) {
                float ch = 0.0F;
                for (char c : info.strF.toCharArray()) {
                ...
                int length = (int) (rect.width() / ch);
                canvas.drawText(info.strF.substring(0, length), rect.centerX(),
                        rect.centerY() + offsetYFestival1, mPaint);
                canvas.drawText(info.strF.substring(length), rect.centerX(),
                        rect.centerY() + offsetYFestival2, mPaint);
            } else {
                canvas.drawText(info.strF, rect.centerX(),
                        rect.centerY() + offsetYFestival1, mPaint);
            }
        }
    }
