...
import com.clinical.tongxin.myview.ExitGroupDialog;
...
//		pushConfigs = EMClient.getInstance().pushManager().getPushConfigs();
	...
	private void getNickname(final List<String> members) {
		progressDialog.show();
		Constant.sExecutorService.execute(new Runnable() {
			...
			public void run() {
				...
				for (String member :members){
					if (TextUtils.isEmpty(SPUtils.getObject(member,""))){
						sb.append(member).append(",");
					}
				}
				if (!TextUtils.isEmpty(sb.toString())){
					requestData(sb.toString().substring(0,sb.toString().length()-1));
				}else {
	...
	private void requestData(String substring) {
		Map map=new HashMap<>();
			...
			public void onSuccess(String arg0) {
				...
				try {
					...
					if (resultEntity.getCode() == 200){
						...
						Toast.makeText(GroupDetailsActivity.this,"获取昵称失败，请重试",Toast.LENGTH_SHORT).show();
					}
...
//				final String[] newmembers = data.getStringArrayExtra("newmembers");
				List<SignEntity> signEntities = (List<SignEntity>) data.getSerializableExtra("newmembers");
				String [] newmembers = new String[signEntities.size()];
				for (int i = 0; i < signEntities.size(); i++) {
					newmembers[i] = signEntities.get(i).getMobile();
				}
				progressDialog.setMessage(st1);
				progressDialog.show();
	...
	public void exitGroup(View view) {
		...
		builder.setMessage(getResources().getString(R.string.exit_group_hint));
...
//		startActivityForResult(new Intent(this, ExitGroupDialog.class), REQUEST_CODE_EXIT);
	...
	public void exitDeleteGroup(View view) {
		...
		builder.setMessage(getResources().getString(R.string.dissolution_group_hint));
		builder.setNegativeButton("取消", new DialogInterface.OnClickListener() {
			@Override
			public void onClick(DialogInterface dialog, int which) {
				dialog.dismiss();
					...
					button.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View v) {
							...
							startActivityForResult(
									(new Intent(GroupDetailsActivity.this, FindSignListActivity.class).putExtra("groupId", groupId)),
...
//				button.setCompoundDrawables(null, avatar, null, null);
				EaseUserUtils.setUserNick(SPUtils.getObject(username,username), holder.textView);
				...
				button.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						if (isInDeleteMode) {
							...
							startActivity(new Intent(GroupDetailsActivity.this, ChatActivity.class).putExtra(EaseConstant.EXTRA_USER_ID, username));
