...
public class NewGroupActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		publibCheckBox.setOnCheckedChangeListener(new OnCheckedChangeListener() {
		    ...
		    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
		        if(isChecked){
		            secondTextView.setText("加入需要群主同意");
		        }else{
	...
	public void save(View v) {
		...
		if (TextUtils.isEmpty(name)) {
			...
			startActivityForResult(new Intent(this, FindSignListActivity.class).putExtra("groupName", name), 0);
		}
	}
		...
		if (resultCode == RESULT_OK && requestCode == 0) {
			...
			new Thread(new Runnable() {
				...
				public void run() {
					...
					String [] members = new String[signEntities.size()];
					for (int i = 0; i < signEntities.size(); i++) {
						members[i] = signEntities.get(i).getMobile();
					}
					try {
						...
						for (SignEntity signEntity:signEntities){
							SPUtils.putObject(signEntity.getMobile(),signEntity.getName());
						}
