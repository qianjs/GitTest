...
public class EaseNotifier {
     ...
     * @param
     */
    public synchronized void onMsgPushGroup(EMMessage message,String content,int id) {
        vibrateAndPlayTone(message);
        try {
...
//            Intent msgIntent = appContext.getPackageManager().getLaunchIntentForPackage(packageName);
            Intent msgIntent = new Intent(appContext, Class.forName("MainActivity"));
            msgIntent.putExtra("msg",message.getBody().toString().substring(message.getBody().toString().indexOf("\"") + 1, message.getBody().toString().lastIndexOf("\"")));
            msgIntent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
            PendingIntent pendingIntent = PendingIntent.getActivity(appContext, id, msgIntent, PendingIntent.FLAG_UPDATE_CURRENT);
            mBuilder.setContentTitle(contentTitle);
            mBuilder.setTicker("系统通知");
...
//        }
    ...
    protected void sendNotification(EMMessage message, boolean isForeground, boolean numIncrease) {
        ...
        try {
            String notifyText = "admin".equals(username)?"我有任务":username + " ";
